...
- 무중단 DB 마이그레이션 리허설 결과를 근거 중심으로 기록해 컷오버 의사결정 정확도를 높인다.
- 전환/롤백 시나리오의 한계와 보완점을 명시해 실제 운영 절차의 재현성을 확보한다.
- 다음 리허설 및 본 전환에서 동일 기준으로 품질을 판정할 수 있도록 판단 기준을 고정한다.

## 비목적

- 애플리케이션 기능 개선이나 비즈니스 로직 변경을 이 문서에서 다루지 않는다.
- 인프라 아키텍처 전체 재설계를 본 보고서의 범위로 포함하지 않는다.
- 단일 실행 로그를 절대적 정답으로 간주하지 않고 반복 검증을 대체하지 않는다.
...
계획서에서는 `SET GLOBAL read_only = ON` 단독으로 write를 차단하는 방식이었다. 그러나 계획서 자체에서 식별한 SUPER 권한 함정(read_only=ON이어도 SUPER 권한 계정은 쓰기 가능)을 해결하기 위해, 설계 단계에서 이중 차단으로 개선했다.
...
이것은 계획서에서 "RDS 전환 후 write가 발생하면 롤백은 데이터 유실을 수반한다"고 정의한 제약의 실증이다. 단방향 복제(역복제 없음)에서 이것은 설계 특성이지 결함이 아니다. 다만, 이 사실은 "롤백이 현실적인 구간은 전환 직후 수 분 이내"라는 계획서의 판단을 뒷받침한다.
...
계획서 에서 정의한 "K6 successCount vs DB count 비교"를 수행했다.
