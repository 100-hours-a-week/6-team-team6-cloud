...
const CUTOVER_READ_PROBE_PATH = __ENV.CUTOVER_READ_PROBE_PATH || '';
const CUTOVER_READ_PROBE_SKIP_AUTH = boolEnv('CUTOVER_READ_PROBE_SKIP_AUTH', false);
const CUTOVER_ALLOW_READ_401 = boolEnv('CUTOVER_ALLOW_READ_401', false);
...
export function setup() {
  ...
  console.log(
    `readProbePath=${CUTOVER_READ_PROBE_PATH || 'auto'} skipAuth=${CUTOVER_READ_PROBE_SKIP_AUTH} allow401=${CUTOVER_ALLOW_READ_401}`,
  );
...
export default function (data) {
  ...
  const readRes = readProbe(data.groupId, headers, 'cutover_read', CUTOVER_READ_PROBE_SKIP_AUTH);
  cutoverResponseTime.add(readRes.timings.duration);

  const readOk = check(readRes, {
    'cutover read status is healthy': (r) => isReadHealthy(r.status),
  });
...
export function timelineProbeScenario(data) {
  ...
  const readRes = readProbe(data.groupId, headers, 'cutover_timeline_read', CUTOVER_READ_PROBE_SKIP_AUTH);
...
function buildHeaders(authToken, skipAuth = false) {
  const headers = {
...
function readProbe(groupId, headers, tagType, skipAuth = false) {
  const requestHeaders = buildHeaders(extractToken(headers), skipAuth);
  ...
  if (CUTOVER_READ_PROBE_PATH) {
    path = CUTOVER_READ_PROBE_PATH;
  }
...
function isReadHealthy(status) {
  if (status >= 200 && status < 300) {
    return true;
  }
  return CUTOVER_ALLOW_READ_401 && status === 401;
}
